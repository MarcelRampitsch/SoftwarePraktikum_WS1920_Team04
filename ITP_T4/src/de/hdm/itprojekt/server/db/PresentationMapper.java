...
/**
 * Die Klasse PresentationMapper bildet <code>Presentation</code> Objekte auf
 * eine relationale Datenbank ab. Ebenfalls ist es möglich aus Datenbank-Tupel
...
public class PresentationMapper {
	 ...
	 * nur eine einzige Instanz der PresentationMapper-Klasse existiert. Aufgerufen
	 * wird die Klasse somit über PresentationMapper.presentationMapper() und nicht
	...
	public Vector<Presentation> findAllByUserID(User u) {
		...
		try { // Prepared Statement erstellen um alle Präsentationen eines bestimmten Cinema
			...
			PreparedStatement findAllByUSerID = con
					.prepareStatement("SELECT * From softwarepraktikum_ws1920.presentation " + "WHERE UserID=? ");
	...
	public Presentation getAllPresentationByPresentationID(SurveyEntry se) {
		...
		try { // Prepared Statement erstellen um alle Präsentationen eines bestimmten Cinema
			...
			PreparedStatement findAllBySurveyEntryID = con.prepareStatement(
					"SELECT * From softwarepraktikum_ws1920.presentation " + "WHERE PresentationID=? ");
	...
	public Presentation findByPresentationID(int id) {
		...
		try {
			...
			// Ergebnis-Tupel in Objekt umwandeln
			p = new Presentation(rs.getString("name"), rs.getInt("cinemaID"), rs.getInt("movieID"), rs.getInt("userID"),
					rs.getInt("timeslotID"), rs.getDate("date"), rs.getInt("id"), rs.getTimestamp("creationDate"));
			// Fehlerbehandlung hinzufügen
		} catch (SQLException e) {
			e.printStackTrace();
			return null;
	...
	public Vector<Presentation> findByDate(int date) {
		...
		Vector<Presentation> result = new Vector<Presentation>();
		try {
			// Prepared Statement erstellen um alle Präsentationen eines bestimmten Date zu
			// finden
			PreparedStatement findByDate = con.prepareStatement("SELECT * From softwarepraktikum_ws1920.presentation "
					+ "WHERE date=? " + "Order By TimeslotID ASC;");
	...
	public Presentation insert(Presentation p) { // Name des Movie/ Cinema ?!?!?!?!
		...
		try {
			...
			PreparedStatement insert = con.prepareStatement(
					"INSERT INTO softwarepraktikum_ws1920.presentation(name, date, userID, cinemaID, timeslotID, movieID) VALUES (?,?,?,?,?,?);");
			...
			PreparedStatement getnewPresentation = con.prepareStatement(
					"SELECT *FROM softwarepraktikum_ws1920.presentation ORDER BY creationDate DESC LIMIT 1;");
			// Ergebnis-Tupel in Objekt umwandeln
			ResultSet rs = getnewPresentation.executeQuery();
			if (rs.next()) {

				return new Presentation(rs.getString("name"), rs.getInt("cinemaID"), rs.getInt("movieID"),
						rs.getInt("userID"), rs.getInt("timeslotID"), rs.getDate("date"), rs.getInt("presentationID"),
						rs.getTimestamp("creationDate"));
			}
			// Fehlerbehandlung hinzufügen
	...
	public Presentation update(Presentation p) {
		...
		try {
			...
			PreparedStatement update = con.prepareStatement(
					"UPDATE softwarepraktikum_ws1920.presentation SET name=?, cinemaID=?, movieID=?, timeslotID=?, date=?  WHERE presentationID=?;");
