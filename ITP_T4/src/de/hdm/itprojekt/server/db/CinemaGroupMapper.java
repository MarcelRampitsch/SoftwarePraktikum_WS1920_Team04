...
//
	 ...
	 * nur eine einzige Instanz der CinemaGroupMapper-Klasse existiert. Aufgerufen
	 * wird die Klasse somit über CinemaGroupMapper.cinemaGroupMapper() und nicht
		...
		try {
			...
			if (c.getId() == 0) {
				PreparedStatement insert = con.prepareStatement(
						"INSERT INTO softwarepraktikum_ws1920.cinemagroup(name, userID) VALUES (?,?);");
			...
			PreparedStatement getnewCinemaGroup = con.prepareStatement(
					"SELECT *FROM softwarepraktikum_ws1920.cinemagroup ORDER BY creationDate DESC LIMIT 1;");
			// Ergebnis-Tupel in Objekt umwandeln
			ResultSet rs = getnewCinemaGroup.executeQuery();
			if (rs.next()) {

				return new CinemaGroup(rs.getInt("cinemaGroupID"), rs.getTimestamp("creationDate"),
	...
	public CinemaGroup findCinemaGroupByID(CinemaGroup cg) {
		...
		try {
			// PreparedStatement erstellen um eine CinemaGroup in die Datenbank einzufügen
			PreparedStatement findCinemaGroupByID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.cinemagroup WHERE cinemaGroupID=?;");

			findCinemaGroupByID.setInt(1, cg.getId());
			ResultSet rs = findCinemaGroupByID.executeQuery();

			// Ergebnis-Tupel in Objekt umwandeln
			cineg = new CinemaGroup(rs.getInt("id"), rs.getTimestamp("creationDate"), rs.getString("name"),
					rs.getInt("userID"));
	...
	public CinemaGroup updateCinemaGroup(CinemaGroup cinemaGroup) {
		...
		try {
			...
			PreparedStatement update = con
					.prepareStatement("UPDATE softwarepraktikum_ws1920.cinemagroup SET name=? WHERE cinemaGroupID=?;");
	...
	public void deleteCinemaGroupByID(int id) {
		...
		try {
			...
			// PreparedStatement erstellen um eine CinemaGroup in die Datenbank einzufügen
			PreparedStatement deleteCinemaGroupById = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.cinemagroup WHERE `cinemaGroupID`=?; ");
	...
	public Vector<CinemaGroup> findcinemagroupbyuserID(User u) {
		...
		try {
			...
			// PreparedStatement erstellen um eine CinemaGroup in die Datenbank einzufügen
			PreparedStatement findCinemaGroupByUserID = con
	...
	public void deleteAllCinemaGroupByUserID(User u) {
		...
		try {
			...
			PreparedStatement deleteAllCinemaGroupByUserID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.cinemagroup WHERE userID=?");
