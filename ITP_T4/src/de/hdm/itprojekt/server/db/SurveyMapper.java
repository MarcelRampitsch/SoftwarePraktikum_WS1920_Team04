...
public class SurveyMapper {
	...
	/**
	 * Die Klasse SurveyMapper wird nur einmal instantiiert (Singleton-Eigenschaft).
	 * Damit diese Eigenschaft erf�llt werden kann, wird zun�chst eine Variable
	 * mit dem Schl�sselwort static und dem Standardwert null erzeugt. Sie
	 ...
	 * dass nur eine einzige Instanz der SurveyMapper-Klasse existiert. Aufgerufen
	 * wird die Klasse somit �ber SurveyMapper.surveyMapper() und nicht �ber den
	 * New-Operator.
	 * 
	...
	public Survey findBySurveyID(Survey s) {
		...
		try {
			...
			// Erstellung des Prepared Statement um alle Surveys anhand der SurveyID zu
			// finden
			PreparedStatement findBySurveyID = con
			...
			while(rs.next()) {
			s = new Survey(rs.getInt("surveyID"), rs.getTimestamp("creationDate"), rs.getString("name"), rs.getInt("userID"),
	...
	public Vector<Survey> findAllSurveyByGroup(Group g) {
		...
		try { // Prepared Statement erstellen um alle Präsentationen eines bestimmten Movies
				// zu finden
			PreparedStatement findAllGroup = con
					.prepareStatement("SELECT * From softwarepraktikum_ws1920.survey " + "WHERE groupID=? ");
	...
	public Survey findByName(Survey s) {
		...
		try {
			PreparedStatement findByName = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.survey WHERE name=?;");
	...
	public Survey insert(Survey s) {
		...
		try {
			PreparedStatement insert = con.prepareStatement(
					"INSERT INTO softwarepraktikum_ws1920.survey(name, groupID, userID) VALUES(?,?,?);");
			...
			PreparedStatement getnewSurvey = con.prepareStatement(
					"SELECT *FROM softwarepraktikum_ws1920.survey ORDER BY creationDate DESC LIMIT 1;");

			ResultSet rs = getnewSurvey.executeQuery();

			if (rs.next()) {
				return new Survey(rs.getInt("surveyID"), rs.getTimestamp("creationDate"), rs.getString("name"),
	...
	public void deleteSurveyBySurveyID(Survey s) {
		...
		try {
			PreparedStatement deleteBySurveyID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.survey WHERE surveyID=?;");
	...
	public Survey updateSurvey(Survey survey) {
		...
		try {
			PreparedStatement update = con.prepareStatement("UPDATE softwarepraktikum_ws1920.survey SET round=? WHERE surveyID=?");

			update.setInt(1, 2);
			update.setInt(2, survey.getId());
	...
	public void deleteAllByUserID(User u) {
		...
		try {
			...
			PreparedStatement deleteAllByUserID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.surveyID WHERE userID =?;");
	...
	public void deleteAllSurveyByGroupID(Group g) {
		...
		try {
			...
			PreparedStatement deleteAllByGroupID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.surveyID WHERE groupID =?;");
