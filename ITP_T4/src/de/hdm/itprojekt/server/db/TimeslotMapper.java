...
public class TimeslotMapper {
	...
	public Timeslot insert(Timeslot t) {
		...
		try {
			...
			PreparedStatement getnewTimeslot = con.prepareStatement(
					"SELECT * FROM softwarepraktikum_ws1920.timeslot ORDER BY creationDate DESC LIMIT 1;");
			// Ergebnis-Tupel in Objekt umwandeln
	...
	public Timeslot update(Timeslot t) {
		...
		try {
			...
			PreparedStatement update = con
					.prepareStatement("UPDATE softwarepraktikum_ws1920.timeslot SET time=? " + " WHERE timeslotID=?;");
	...
	public Vector<Timeslot> findAllTimeslotByUserID(User u) {
		...
		try { // Prepared Statement erstellen um alle Präsentationen eines bestimmten Cinema
			...
			PreparedStatement findAllByUSerID = con
					.prepareStatement("SELECT * From softwarepraktikum_ws1920.timeslot " + "WHERE UserID=? ");
