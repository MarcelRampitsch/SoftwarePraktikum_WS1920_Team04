...
/**
 * Die Klasse VoteMapper bildet <code>Vote</code> Objekte auf eine relationale
...
public class VoteMapper {
	...
	/**
	 * Die Klasse VoteMapper wird nur einmal instantiiert (Singleton-Eigenschaft).
	 ...
	 * nur eine einzige Instanz der VoteMapper-Klasse existiert. Aufgerufen wird die
	 * Klasse somit über VoteMapper.voteMapper() und nicht über den New-Operator.
	 * 
	...
	public Vote findVoteByVoteID(int id) {
		...
		try {
			...
			PreparedStatement findVoteByUserID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.vote" + "WHERE voteID=?");
	...
	public Vector<Vote> findVoteByUserID(int userID) {
		...
		try {
			...
			PreparedStatement findVoteByUserID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.vote" + "WHERE userID=?");
	...
	public Vote insert(Vote v) {
		...
		try {
			...
			PreparedStatement getnewVote = con.prepareStatement(
					"SELECT * FROM softwarepraktikum_ws1920.vote ORDER BY creationDate DESC LIMIT 1;");
	...
	public Vote update(Vote v) {
		...
		try {
			...
			PreparedStatement update = con
					.prepareStatement("UPDATE softwarepraktikum_ws1920.vote SET voteResult  WHERE voteID=?;");
