...
public class UserMapper {
	...
	public User findByUserID(User user) {
		...
		try {
			...
			PreparedStatement findByUserID = con
			...
			while (rs.next()) {
				u = new User(rs.getInt("userID"), rs.getString("nickname"), rs.getString("email"),
	...
	public User findByNickname(User user) {
		...
		try {
			...
			PreparedStatement findBySurveyID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.user WHERE nickname=?");
	...
	public User findByEmail(User u) {
		...
		try {
			PreparedStatement findByEmail = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.user " + "WHERE email=?");
			findByEmail.setString(1, u.getEmail());

			ResultSet rs = findByEmail.executeQuery();
			if (rs.next()) {
				user = new User(rs.getInt("userID"), rs.getString("nickname"), rs.getString("email"),
						rs.getTimestamp("creationDate"));
			}
	...
	public User insert(User u) {
		...
		try {
			PreparedStatement insert = con
					.prepareStatement("INSERT INTO softwarepraktikum_ws1920.user(nickname, email) VALUES(?,?);");
			...
			PreparedStatement getnewUser = con
					.prepareStatement("SELECT *FROM softwarepraktikum_ws1920.user ORDER BY creationDate DESC LIMIT 1;");
	...
	public void deleteUserByUserID(User u) {
		...
		try {
			PreparedStatement deleteByUserID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.user WHERE userID=?;");
