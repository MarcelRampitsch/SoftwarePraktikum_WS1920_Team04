...
public class MovieMapper {
	...
	/**
	 * Die Klasse MovieMapper bildet <code>Movie</code> Objekte auf eine relationale
	...
	/**
	 * Die Klasse MovieMapper wird nur einmal instantiiert (Singleton-Eigenschaft).
	 ...
	 * dass nur eine einzige Instanz der MovieMapper-Klasse existiert. Aufgerufen
	 * wird die Klasse somit �ber MovieMapper.movieMapper() und nicht �ber den
	 * New-Operator.
	 * 
	...
	public Movie findByMovieID(Movie m) {
		...
		try {
			...
			PreparedStatement findByMovieID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.movie WHERE movieID=? ;");
	...
	public Movie findByName(Movie m) {
		...
		try {
			...
			PreparedStatement findByName = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.movie WHERE name=?;");
	...
	public Vector<Movie> findAllByUserID(User u) {
		...
		try {
			PreparedStatement findAllByUserID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.movie " + "WHERE UserID=? ");
	...
	public Movie insert(Movie m) {
		...
		try {
			...
			PreparedStatement insert = con
					.prepareStatement("INSERT INTO softwarepraktikum_ws1920.movie(name, userID) VALUES(?,?);");
			...
			// Erstellung des PreparedStatement getnewMovie (W�hlt den zuletzt erstellten
			// Film absteigend).
			PreparedStatement getnewMovie = con.prepareStatement(
					"SELECT * FROM softwarepraktikum_ws1920.movie ORDER BY creationDate DESC LIMIT 1;");

			ResultSet rs = getnewMovie.executeQuery();

			if (rs.next()) {
				return new Movie(rs.getInt("movieID"), rs.getTimestamp("creationDate"), rs.getString("name"),
						rs.getInt("userID"));
			}
	...
	public Movie updateMovie(Movie m) {
		...
		try {
			PreparedStatement updateMovie = con
					.prepareStatement("UPDATE softwarepraktikum_ws1920.movie SET name=? WHERE movieID=?;");
	...
	public void deleteByMovieID(Movie m) {
		...
		try {
			...
			PreparedStatement deleteByMovieID = con
	...
	// Methode zum l�schen aller von einem Anwender erstellten Filme aus der
	// Datenbank
	public void deleteAllByUserID(User u) {
		...
		try {
			PreparedStatement deleteAllByUserID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.movie WHERE userID =?;");
