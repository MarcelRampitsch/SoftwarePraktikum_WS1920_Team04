...
public class SurveyEntryMapper {
	 ...
	 * nur eine einzige Instanz der SurveyEntryMapper-Klasse existiert. Aufgerufen
	 * wird die Klasse somit über SurveyEntryMapper.surveyEntryMapper() und nicht
	...
	public SurveyEntry findSurveyEntryBySurveyEntryID(SurveyEntry se) {
		...
		try {
			...
			PreparedStatement findSurveyEntryBySurveyEntryID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.surveyentry" + "WHERE surveyEntryID=?");
			...
			while (rs.next()) {
				...
				se = new SurveyEntry(rs.getInt("id"), rs.getTimestamp("creationDate"), rs.getInt("surveyID"),
						rs.getInt("presentationID"));
	...
	public SurveyEntry findSurveyEntryBySurveyID(Survey s) {
		...
		try {
			...
			PreparedStatement findSurveyEntryBySurveyID = con
					.prepareStatement("SELECT * FROM softwarepraktikum_ws1920.surveyentry" + "WHERE surveyID=?");
	...
	public SurveyEntry insert(SurveyEntry se) {
		...
		try {
			...
			PreparedStatement insert = con.prepareStatement(
					"INSERT INTO softwarepraktikum_ws1920.surveyentry(surveyID, presentationID) VALUES(?,?);");
			...
			PreparedStatement getnewSurveyEntry = con.prepareStatement(
					"SELECT * FROM softwarepraktikum_ws1920.surveyentry ORDER BY creationDate DESC LIMIT 1;");

			ResultSet rs = getnewSurveyEntry.executeQuery();

			if (rs.next()) {

				return new SurveyEntry(rs.getInt("id"), rs.getTimestamp("creationDate"), rs.getInt("surveyID"),
						rs.getInt("presentationID"));
	...
	public SurveyEntry update(SurveyEntry se) {
		...
		try {
			...
			PreparedStatement update = con.prepareStatement(
					"UPDATE softwarepraktikum_ws1920.surveyentry SET presentationID=?  WHERE surveyEntryID=?;");
	...
	public void deleteAllByUserID(User u) {
		...
		try {
			PreparedStatement deleteAllByUserID = con
					.prepareStatement("DELETE FROM softwarepraktikum_ws1920.surveyentry WHERE userID=?;");
